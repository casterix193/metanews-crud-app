...
const EditArticleForm = ({ article, formData, setFormData }) => {
  const router = useRouter();
  const queryClient = useQueryClient();

  const updateMutation = useMutation(
    (newData) => Articles.update(article._id, newData),
    {
      onSuccess: () => {
      ...
      onError: (error) => {
        ...
        toast.error("Sorry. Some error occurred while updating article");
      },
    }
  );
        ...
        <div className="col-span-2 flex gap-4 items-center">
          ...
          <div className="form-check">
            <input
              ...
              id="radioDefault2"
              name="status"
              defaultChecked={article.status === "inactive"}
